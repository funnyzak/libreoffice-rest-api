...
# LibreOffice-REST-API Installation & Management Script
# GitHub: https://github.com/funnyzak/libreoffice-rest-api
# Usage: curl -fsSL https://raw.githubusercontent.com/funnyzak/libreoffice-rest-api/main/scripts/install.sh | bash
...
# Configuration
GITHUB_REPO="funnyzak/libreoffice-rest-api"
...
print_banner() {
    echo ""
    title "LibreOffice-REST-API Installation & Management Tool"
...
Commands:
    install         Install LibreOffice-REST-API (default if no command specified)
    uninstall       Uninstall LibreOffice-REST-API
...
show_post_install() {
    ...
    echo ""
    success "LibreOffice-REST-API has been installed!"
    echo ""
...
cmd_install() {
    echo ""
    title "Installing LibreOffice-REST-API"
    echo ""
    ...
    if existing_binary=$(find_installed_binary); then
        ...
        warn "LibreOffice-REST-API is already installed: $existing_binary"
...
# Uninstall LibreOffice-REST-API
cmd_uninstall() {
    echo ""
    title "Uninstalling LibreOffice-REST-API"
    echo ""

    # Find binary
    local binary_path
    if ! binary_path=$(find_installed_binary); then
        warn "LibreOffice-REST-API is not installed or not found in common locations"
        echo ""
    ...
    echo ""
    success "LibreOffice-REST-API has been uninstalled successfully"
}

# Update LibreOffice-REST-API
cmd_update() {
    echo ""
    title "Updating LibreOffice-REST-API"
    echo ""

    # Check if installed
    local binary_path current_version latest_version
    if ! binary_path=$(find_installed_binary); then
        warn "LibreOffice-REST-API is not installed"
        echo ""
...
cmd_check() {
    echo ""
    title "Checking LibreOffice-REST-API Installation"
    echo ""
    ...
    if binary_path=$(find_installed_binary); then
        current_version=$(get_installed_version) || current_version="unknown"
        success "LibreOffice-REST-API is installed"
...
interactive_menu() {
    ...
    while true; do
        ...
        echo "  1) Install LibreOffice-REST-API"
        echo "  2) Uninstall LibreOffice-REST-API"
        echo "  3) Update LibreOffice-REST-API"
